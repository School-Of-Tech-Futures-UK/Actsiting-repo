...
import { useParams } from "react-router-dom";
  ...
  return (
    ...
    <div className='venue-summary-page-body'>
      ...
      <div>{events.map(event =><div class='event-details'><strong key={event.venue_id}>Venue ID:</strong> {event.venue_id} , <strong>Event ID:</strong> {event.event_id}, <strong>Event Name:</strong> {event.event_name}, <strong>Event Date:</strong> {event.date} , <strong>Event Booking Status:</strong> {event.status} <button id="accept-button" onClick={()=> acceptEventRequest(event.event_id)}>Accept</button> <button id="decline-button" onClick={() => declineEventRequest(event.event_id)}>Decline</button></div>)}</div>
