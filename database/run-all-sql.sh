...
export DB_SERVER=${1:-"localhost"}
export DB_NAME=postgres # default one
export DB_PORT=5432 # default one

# USING A ENV VAR FOR THIS IS NOT SECURE, MKAY!!!!
 export DB_PWD=${POSTGRES_PASSWORD:-"mysecretpassword"} # from the other script
# ^^ the space befoe keeps it out of your bash history though
...
psql "postgresql://${DB_USER}:${DB_PWD}@${DB_SERVER}/${DB_NAME}" --file select-data.sql --echo-all
